...
const char* pszBases[FSR_Count] = {
	"Examples/01_Transformations/",     // FSR_BinShaders
	"Examples/01_Transformations/",     // FSR_SrcShaders
	"Examples/UnitTestResources/",          // FSR_Textures
	"Examples/UnitTestResources/",          // FSR_Meshes
	"Examples/UnitTestResources/",          // FSR_Builtin_Fonts
	"Examples/01_Transformations/",     // FSR_GpuConfig
	"",                                     // FSR_Animation
	"",                                     // FSR_OtherFiles
	"",    // FSR_MIDDLEWARE_TEXT
	"",      // FSR_MIDDLEWARE_UI
...
void TF::reset( uint32_t width, uint32_t height, WindowsDesc *desc )
{
	mWidth = width;
	mHeight = height;
	mWindowsDesc = desc;
...
void TF::update( float dt )
{
	...
	const float aspectInverse = (float)mHeight / (float)mWidth;
...
bool TF::addSwapChain()
{
	SwapChainDesc swapChainDesc = {};
	swapChainDesc.pWindow = mWindowsDesc;
	swapChainDesc.mPresentQueueCount = 1;
	swapChainDesc.ppPresentQueues = &pGraphicsQueue;
	swapChainDesc.mWidth = mWidth;
	swapChainDesc.mHeight = mHeight;
...
bool TF::addDepthBuffer()
{
	...
	depthRT.mHeight = mHeight;
	depthRT.mSampleCount = SAMPLE_COUNT_1;
	depthRT.mSampleQuality = 0;
	depthRT.mWidth = mWidth;
	addRenderTarget(pRenderer, &depthRT, &pDepthBuffer);

	return pDepthBuffer != NULL;
}

void TF::recenterCameraView(float maxDistance, vec3 lookAt)
{
...
bool TF::cameraInputEvent(const ButtonData* data)
{
